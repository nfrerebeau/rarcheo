...
Ce livre vise à résoudre un problème spécifique : la majorité des archéologues reçoivent peu ou pas de formation en programmation scientifique pour l'analyse et la visualisation des données, bien qu'il s'agisse de tâches courantes en archéologie. Un programme d'interface de ligne de commande tel que R est idéal pour ce type de travail, et pourtant il est peu familier et exotique pour la plupart des archéologues. Une interface de ligne de commande consiste à interagir avec un logiciel en envoyant des instructions au programme sous forme de lignes de texte brut. Au lieu de cela, la plupart des archéologues utilisent Microsoft Excel, SPSS ou un logiciel commercial similaire de type "pointer-cliquer". Cela pose quatre problèmes.

1. Il en résulte généralement beaucoup de répétitions fastidieuses, comme le copier-coller entre des feuilles de calcul, ou entre un tableur et un logiciel de traitement de texte. Ces dernières années, les biologistes ont vu le volume des données génomiques augmenter considérablement, en raison des progrès de la technologie de séquençage. Cela les a amenés à rechercher des moyens plus efficaces d'analyser leurs données et d'automatiser leurs analyses. Ils ont constaté que les tableurs n'offraient pas suffisamment de souplesse pour mener à bien leurs analyses. Par conséquent, beaucoup se sont tournés vers R, Python et d'autres langages de programmation similaires pour surmonter les limites et les inefficacités des tableurs. Le détail essentiel ici est le passage de l'interface de type pointer-cliquer d'un tableur à l'interface de type ligne de commande d'un langage de programmation, où des instructions sont fournies à l'ordinateur en texte brut.

2. Cela limite le développement de nouvelles méthodes, car l'utilisateur est limité à ce qui est disponible dans le logiciel commercial. Avec la plupart des logiciels commerciaux, vous êtes limité à l'ensemble des fonctions que l'éditeur choisit de mettre à votre disposition. Avec un langage de programmation sous [licence libre](#pourquoi-travailler-avec-r), vous et toute autre personne êtes libres de mettre en œuvre de nouvelles méthodes. R dispose de fonctions étendues pour l'analyse des données, mais plus important encore, R dispose d'un large [éventail d'extensions](#pourquoi-travailler-avec-r) pour l'analyse quantitative, la visualisation, l'import et la manipulation des données. Pour la plupart des archéologues, quelle que soit l'analyse ou le graphique que vous cherchez à réaliser, il y a de fortes chances que quelqu'un l'ait déjà essayé, ou quelque chose de très similaire, et ait rendu le code disponible. R est également la *lingua franca* des chercheurs en statistiques, qui publient souvent une extension à R pour accompagner leurs articles scientifiques. Cela signifie souvent un accès immédiat aux toutes dernières techniques et implémentations statistiques.

3. Les logiciels commerciaux (et même certains logiciels libres) rendent difficile la démonstration et la reproductibilité en raison de la nature éphémère des interfaces de type pointer-cliquer. Ces interfaces sont familières et faciles à utiliser pour la plupart des gens, car elles sont associées au développement de l'informatique domestique. Il est cependant très difficile d'enregistrer efficacement une séquence de clics complexes afin qu'une autre personne (ou vous-même à l'avenir) puisse répéter la procédure sans ambiguïté. Il est donc difficile de rendre vos analyses reproductibles si tout votre travail est effectué avec une souris. Il n'est pas pratique d'abandonner complètement l'utilisation de la souris lorsqu'on utilise un ordinateur, mais en utilisant un programme en ligne de commande tel que R, il devient possible d'enregistrer les étapes les plus importantes de l'analyse en texte brut. Nous disposons alors d'un enregistrement détaillé des étapes de notre analyse : nous pouvons facilement la partager avec d'autres et la réutiliser nous-mêmes, des mois ou des années plus tard.

4. Les logiciels commerciaux limitent la transparence de la recherche, car les algorithmes qui sous-tendent les fonctions utilisées ne peuvent pas être inspectés et les modifications ne sont pas autorisées. Les statisticiens ont noté depuis longtemps qu'Excel comporte des procédures statistiques inexactes [@mccullough2008; @yalta2008], et les manuels d'introduction aux statistiques avertissent les étudiants de ne pas utiliser Excel lorsque les résultats sont importants [voir par exemple @keller2000]. Avec un logiciel libre tel que R, vous pouvez facilement inspecter et modifier les algorithmes derrière chaque opération. R, en particulier, présente l'avantage supplémentaire d'être l'un des logiciels les plus précis pour l'analyse statistique [@almiron2009; @keeling2007].
...
Ce livre est complémentaire aux [principaux manuels d'archéologie quantitative](#ressources-complémentaires). Ces ouvrages sont excellents pour la théorie, la discussion et les exemples statistiques pertinents, et ce livre ne les remplace pas sur votre étagère. Cependant, ces livres ont trois limites que nous aborderons dans les chapitres suivants.

1. La première de ces limites est l'absence d'exemples prêts à l'emploi, qui oblige le lecteur à déployer des efforts considérables pour mettre en œuvre une méthode décrite à l'aide de formules, ce qui crée de nombreuses possibilités d'erreur. @vanpool2010 demandent ainsi à leurs lecteurs de faire leurs analyses statistiques à la main, une recommandation peu pratique et dépassée alors que les ordinateurs sont facilement disponibles. La quantité d'efforts requis par ces textes est prohibitive pour l'exploration rapides des données et l'expérimentation à l'aide de nouvelles méthodes. Ce livre comprend des exemples reproductibles utilisant des jeux de données réels, de sorte que vous pouvez facilement procéder à l'analyse pour explorer les effets de différents paramètres, et échanger facilement vos propres données.

2. Les ouvrages existants n'accordent que peu de place aux méthodes intuitives et méthodologiquement plus robustes, telles que le rééchantillonnage et les analyses bayésiennes, privilégiant les méthodes paramétriques traditionnelles. Cela reflète une époque où la puissance de calcul était une ressource rare et où les calculs étaient souvent effectués à la main ou à l'aide d'une calculatrice [voir par exemple @fletcher1991]. Les ordinateurs modernes peuvent maintenant facilement gérer le rééchantillonnage et les méthodes bayésiennes, et R est unique en ce qu'il possède un ensemble mature de méthodes pour calculer les tests courants dans ces cadres. Ces méthodes sont également de plus en plus courantes dans la littérature scientifique. Ce livre présente ces approches alternatives afin de vous permettre de développer votre boîte à outils analytique.

3. Les ouvrages actuellement disponibles sont muets sur les aspects pratiques de nombreuses tâches d'analyse et de visualisation de données, courantes en archéologie bien qu'elles ne sont pas traditionnellement considérées comme des statistiques. Il s'agit notamment de la visualisation et de l'analyse d'informations stratigraphiques issues de fouilles et de données spatiales provenant d'enquêtes. L'apprentissage d'outils pour ces tâches se fait traditionnellement en autodidacte, et souvent à grands frais avec des logiciels propriétaires. Ce livre montre comment écrire des programmes simples et utiles pour les archéologues. Ce livre essaie ainsi de répondre à un besoin de formation à l'usage d'outils libres pour la réalisation de tâches courantes en archéologie. Pour ce faire, il présente un environnement de travail reproductible afin de montrer comment R et ses extensions peuvent être utilisés pour mener de bout en bout sa recherche à partir de données de prospection, de fouille et de laboratoire.

## Pourquoi travailler avec R ? {-}

Si vous êtes novice en matière de R, vous vous demandez peut-être quel est l'attrait de ce logiciel, surtout quand il existe tant de programmes et de langages couramment utilisés. Voici quelques-unes des caractéristiques qui nous ont incité à apprendre R :
...
: R propose des outils flexibles pour communiquer vos résultats, en permettent de produire facilement des documents complexes comprenant du texte, des tableaux et des figures, le tout généré à partir du code (comme ce livre !). C'est un gain de temps considérable, surtout lorsque des semaines ou des mois s'écoulent entre deux projets et que le souvenir des détails du dernier travail s'estompe. Lorsque je consulte mon code R, je peux rapidement me souvenir de mon plan d'analyse et reprendre là où je me suis arrêté.
