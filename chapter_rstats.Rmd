...
Essayez vous-même en exécutant les commandes suivantes pour installer certains packages que nous utiliserons dans les chapitres suivants. Vous ne devez utiliser `install.packages()` qu'une seule fois (par ordinateur et par utilisateur) : vous n'avez pas besoin de le faire à chaque nouvelle session de travail. Remarquez comment nous utilisons la fonction `c()` ici pour créer un vecteur de caractères de noms de packages sur lequel la fonction `install.packages()` peut travailler :
...
```{r factominer-dependency, echo=FALSE, fig.width=7, fig.height=7, fig.cap="(ref:factominer-dependency)"}
...
```{r r-atomic, echo=FALSE, fig.width=5, fig.height=1, out.width='33%', fig.cap="Un vecteur est un ensemble d'éléments de même type."}
vec <- data.frame(x = 1:5, y = 1, z = "A")
ggplot(data = vec) +
  aes(x = x, y = y, fill = z, label = x) +
  geom_tile(colour = "white", size = 2) +
  geom_text(size = 6) +
  theme_void() +
  theme(legend.position = "none") +
  scale_fill_manual(values = c(A = "#EECC66"))
```
...
```{r r-list, echo=FALSE, fig.width=5, fig.height=1, out.width='33%', fig.cap="Une liste est un vecteur dont les éléments peuvent être de différents types."}
...
```{r r-matrix, echo=FALSE, fig.width=5, fig.height=5, out.width='33%', fig.cap="Une matrice (gauche) est un tableau à deux dimensions dont tous les éléments sont de même type. Un array (droite) peut être vu comme un empilement de matrices (formant ainsi un cube de données).", fig.show='hold'}
...
ggplot(data = rbind(vec, col, row)) +
  aes(x = x, y = y, fill = z, label = i) +
  geom_tile(colour = "white", size = 2) +
  geom_text(size = 6) +
  theme_void() +
  theme(legend.position = "none") +
  scale_fill_manual(values = c(A = "#EECC66", X = "#997700"))

ggplot(data = rbind(vec, col, row)) +
  aes(fill = z, label = i) +
  geom_tile(aes(x = x, y = y), colour = "white", size = 2) +
  geom_tile(aes(x = x + 0.5, y = y -0.5), colour = "white", size = 2) +
  geom_tile(aes(x = x + 1, y = y - 1), colour = "white", size = 2) +
  # geom_text(aes(x = x + 1, y = y - 1), size = 6) +
  theme_void() +
  theme(legend.position = "none") +
  scale_fill_manual(values = c(A = "#EECC66", X = "#997700"))
```
...
```{r r-dataframe, echo=FALSE, fig.width=5, fig.height=5, out.width='33%', fig.cap="Un data.frame est un tableau à deux dimensions dont les colonnes peuvent être de différents types."}
...
::: {.float-right}
```{r flow-if, echo=FALSE, out.width='100%', fig.cap="Diagramme d'un test \"si...\"", fig.env='marginfigure'}
DiagrammeR::grViz(
  'digraph {
  splines = ortho
...
::: {.float-right}
```{r flow-ifelse, echo=FALSE, out.width='100%', fig.cap="Diagramme d'un test \"si... sinon...\"", fig.env='marginfigure'}
DiagrammeR::grViz(
  'digraph {
  splines = ortho
...
::: {.float-right}
```{r flow-elseif, echo=FALSE, out.width='100%', fig.cap="Diagramme d'un test \"sinon... si...\"", fig.env='marginfigure'}
DiagrammeR::grViz(
  'digraph {
  splines = ortho
...
::: {.float-right}
```{r flow-while, echo=FALSE, out.width='100%', fig.cap="Diagramme d'une boucle à précondition.", fig.env='marginfigure'}
DiagrammeR::grViz(
  'digraph {
  splines = ortho
