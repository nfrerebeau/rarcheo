...
Un fichier peut être désigné grâce à son **chemin d'accès**, c'est-à-dire une chaîne de caractères décrivant la position de ce fichier dans la hiérarchie du système de fichiers^[Il est également possible d'établir une **connexion** sur un fichier, mais cet aspect n'est traité ici (pour le moment).]. R fournir de nombreux outils pour manipuler des fichiers ou des chemin d'accès :

```{r}
# Informations sur un fichier
file.info("data/isotopes.csv", extra_cols = FALSE)

# Construit un chemin d'accès indépendamment de la plateforme utilisée
file.path("chemin", "vers", "mon", "fichier")

# Test l'existence d'un fichier
file.exists("mon/fichier/est/absent.txt")
...
dirname("chemin/vers/mon/fichier.txt")
```
