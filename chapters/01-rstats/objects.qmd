...
Les `data.frame` permettent facilement de :

* Stocker les données associées dans une structure de données de type tableau ;
* Modifier, extraire, trier l'ensemble des données d'une manière confortable ;
* Simplifier l'analyse complexe des grands ensembles de données.

Dans un `data.frame` :

* Les colonnes doivent avoir un nom ;
* Les noms des lignes doivent être uniques ;
* Chaque colonne peut être de type différent ;
* Chaque colonne doit contenir le même nombre de valeurs.
...
Lorsque l'on travaille avec des tableaux de grande taille, il est possible (et plus commode) d'obtenir une vue d'ensemble de nos données à l'aide de la fonction `str()`:
